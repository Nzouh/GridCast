...
\textbf{Selected model:} A \textbf{Temporal Fusion Transformer} (\tft), implemented via
the \texttt{pytorch-forecasting} library. The \tft was chosen for three reasons: it
...
Output quantiles    & 0.1, 0.25, 0.5, 0.75, 0.90 \\
...
\begin{tikzpicture}[
  ...
  \node[box, fill=gray!15, below=0.4cm of eia]  (om)  {Open-Meteo\\(ensemble)};
  \node[box, fill=gray!15, below=0.4cm of om]   (gs)  {Grid Status\\(LMP)};

  % Backend
  \node[box, fill=blue!12, right=1.8cm of om]   (api)  {FastAPI\\(Code Engine)};

  % Storage + model
  \node[box, fill=orange!15, above right=0.2cm and 1.5cm of api] (cos) {IBM COS\\(model + data)};
  \node[box, fill=orange!15, below right=0.2cm and 1.5cm of api] (wxa) {watsonx.ai\\(prod serving)};

  % Frontend
  \node[box, fill=green!12, right=1.5cm of api, yshift=0cm] (fe) {React\\(Vercel)};

  % Arrows
  \draw[arr] (eia) -- (api);
  \draw[arr] (om)  -- (api);
  \draw[arr] (gs)  -- (api);
  \draw[arr] (cos) -- (api) node[midway, above, font=\tiny] {weights};
  \draw[arr] (api) -- (wxa) node[midway, right, font=\tiny] {inference\\(prod)};
  \draw[arr] (api) -- (fe)  node[midway, above, font=\tiny] {JSON};
\end{tikzpicture}
\caption{GridCast system architecture. Solid arrows show data flow at inference time.}
\label{fig:arch}
\end{figure}
...
The build is organised into seven phases. Phases 1a and 6 can run in parallel once
Phase 0 is complete. Phase 3 (training) is the longest serial step and blocks Phase 4.
...
\midrule
1a & ERA5 download
   & \texttt{cdsapi}: 2m temp, 10m wind, solar radiation; bounding
